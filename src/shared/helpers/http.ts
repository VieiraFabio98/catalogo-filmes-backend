...
export const conflictError = (type: string, message?: string): HttpResponse<Error> => ({
  statusCode: 409,
  data: (new ConflictError(type, message)).error
})
...
export const serverErrorMessage = <T = any> (error: T): HttpResponse<T> => ({
  statusCode: 500,
  data: null,
  error,
})
